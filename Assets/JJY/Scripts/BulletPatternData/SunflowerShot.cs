...
[CreateAssetMenu(fileName = "SunflowerShot", menuName = "ScriptableObject/BulletPattern/SunflowerShot")]
        ...
        for (int j = 0; j < sunflowerCount; j++)
        {
            ...
            for (int i = 0; i < shotCount; i++)
            {
                ...
                if (bulletPrefab != null)
                {
                    ...
                    if (twistThisSunflower)
                    {
                        float twistAngle = (idx % 2 == 0) 
                            ? twistAnglePerSunflower 
                            : -twistAnglePerSunflower;

                        firePoints[idx].Rotate(0f, twistAngle, 0f);
                    }
            ...
            yield return new WaitForSeconds(fireDelayBetweenSunflower);
        }
    }
}
